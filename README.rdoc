...
One can add it at runtime by POSTing it to /rest/1.0:
    ...
    curl -v -XPOST http://127.0.0.1:8080/rest/1.0 -H'Content-type: application/json' -d@newStream.json

== Exposing data over HTTP

Meteo ships with a special publisher (com.ning.metrics.meteo.publishers.ResourceListener) to expose data over HTTP (jsonp). This is useful for graphing purposes (see https://github.com/pierre/metrics-dashboard for an example on how to use it with d3.js).
