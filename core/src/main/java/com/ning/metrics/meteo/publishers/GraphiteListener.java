...
class GraphiteListener implements UpdateListener
{
    ...
    public GraphiteListener(GraphitePublisherConfig config)
    {
        this.config = config;
        this.publisher = new GraphitePublisher(config);

        publisher.failSafeConnect();
    }
        ...
        if (newEvents != null) {
            for (EventBean newEvent : newEvents) {
                for (String attribute : newEvent.getEventType().getPropertyNames()) {
                    publisher.send(config.getPrefix() + "." + attribute, newEvent.get(attribute));
                }
            }
        }
    }
